...
description: Gestor centralizado de fondos oscuros para modales y overlays con contador de referencias múltiple
---
...
El sistema de fondos modales proporciona un gestor centralizado para crear y controlar fondos oscuros que aparecen detrás de modales, menús desplegables y otros overlays. Utiliza un sistema de contador de referencias que permite múltiples componentes compartir el mismo fondo sin conflictos.
...
// El fondo aparece automáticamente
...
```html
<!-- El fondo se crea automáticamente en el DOM -->
...
Sistema único que maneja un fondo modal global compartido por todos los componentes que lo necesiten.

```javascript
// Múltiples componentes pueden usar el mismo fondo
function abrirModal() {
...
### Control de Scroll
Automáticamente bloquea y restaura el scroll del body cuando el fondo está activo.

```css
/* Cuando el fondo está activo */
body.noScroll {
    overflow: hidden;
}
```
...
- **z-index**: `999` (alto para estar sobre la mayoría del contenido)
- **Cursor**: `pointer` para indicar interactividad

### Estados del Fondo
- **Oculto**: `display: none` cuando contador = 0
- **Visible**: `display: block` cuando contador > 0
- **Transición**: Sin animaciones (cambio instantáneo)

### Estructura HTML Generada

```html
<!-- Fondo modal global (creado automáticamente) -->
...
#fondoModalGlobal {
    ...
    transition: opacity 0.3s ease; /* Añadir transición */
}
...
- ✅ **Compartible**: Múltiples componentes pueden usar el mismo fondo
- ✅ **Thread-safe**: Sistema de contador evita conflictos de referencias
- ✅ **Event-driven**: Comunicación desacoplada mediante eventos
- ✅ **Performance**: Elemento único reutilizado, no crea múltiples fondos
...
4. **Múltiples fondos**: Solo debe existir un elemento con `id="fondoModalGlobal"`
...
### Casos Edge
```javascript
// Si el script se recarga múltiples veces
...
const validarSistema = () => {
    ...
    if (!fondo && contador > 0) {
        console.error('Error crítico: contador > 0 pero no hay elemento fondo');
    }
};
```
