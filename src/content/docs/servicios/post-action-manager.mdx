...
description: Utilidades completas para operaciones CRUD de posts en WordPress con validación y logging
---
...
El `PostActionManager` proporciona una interfaz unificada y segura para realizar operaciones CRUD (Crear, Leer, Actualizar, Eliminar) en posts de WordPress. Incluye validación automática, manejo de errores, logging detallado y integración con el sistema de eventos.

## Funcionalidades Principales

### ✏️ Operaciones CRUD Completas
- **Creación de posts**: Con validación de tipos y datos
- **Actualización inteligente**: Preserva datos existentes, actualiza solo campos necesarios
- **Eliminación segura**: Con verificación de permisos y estados
- **Movimiento a papelera**: Operaciones reversibles

### 🔍 Validación y Seguridad
- **Validación de tipos**: Verifica existencia de post types
- **Control de permisos**: Solo usuarios autorizados pueden modificar
- **Sanitización automática**: Limpieza de datos de entrada
- **Prevención de conflictos**: Estados de procesamiento seguros

### 📊 Consultas Avanzadas
- **Búsqueda por ID**: Recuperación eficiente por identificador
- **Búsqueda por slug**: Localización por URL amigable
- **Verificación de existencia**: Métodos múltiples de validación
...
$postId = PostActionManager::crearPost('post', [
    'post_title' => 'Mi Nuevo Artículo',
    'post_content' => 'Contenido del artículo...',
    'post_status' => 'publish'
]);

// Crear una página con metadatos
$pageId = PostActionManager::crearPost('page', [
    'post_title' => 'Página de Contacto',
    'post_content' => 'Información de contacto...',
    'meta_input' => [
...
// Eliminación normal (va a papelera)
...
El manager emite eventos automáticamente a través de `EventBus`:
...
PostActionManager::updatePost(['ID' => 123, 'post_title' => 'Nuevo Título']);
// Emite: 'post_producto'
```
...
EventBus::on('post_producto', function($data) {
    // Lógica personalizada cuando se actualiza un producto
...
$erroresPosibles = [
    'invalid_post_type'    => 'Tipo de post no válido o inexistente',
    'missing_title'        => 'Título del post requerido',
    'missing_id'           => 'ID del post requerido para actualización',
...
```php
// Logging automático para todas las operaciones
...
function limpiarPostsAntiguos($dias = 30) {
    ...
    foreach ($postsAntiguos as $post) {
        PostActionManager::deletePost($post->ID, true); // Eliminación permanente
    }
}
```
...
Crea un nuevo post con validación completa.
...
### Permisos Requeridos
- `edit_post` para actualización
- `delete_post` para eliminación
...
// Operaciones directas también disponibles
...
- Liberación de objetos después del uso
...
```php
// Verificar EventBus está disponible
