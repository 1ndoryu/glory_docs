...
description: Servicio de gestión automática de CSS crítico para optimización de rendimiento web
sidebar:
...
```mermaid
graph TD
    A[Usuario solicita página] --> B{GestorCssCritico::getParaPaginaActual}
    B --> C{CSS en cache?}
    C -->|Sí| D[Servir CSS cacheado]
    C -->|No| E{Generación automática habilitada?}
    E -->|Sí| F[Programar generación en background]
    E -->|No| G[Servir sin CSS crítico]
