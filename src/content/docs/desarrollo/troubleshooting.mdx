...
description: Guía de resolución de problemas comunes durante el desarrollo con Glory Framework
---

# 🔧 Solución de Problemas

Esta guía te ayudará a resolver los problemas más comunes que pueden surgir durante el desarrollo con Glory Framework. Si encuentras un problema que no está cubierto aquí, revisa los logs de Glory o consulta la documentación específica del componente.

## 📋 Tabla de Contenidos

- [🚀 Problemas de Instalación](#problemas-de-instalacion)
- [🎨 Problemas con Componentes](#problemas-con-componentes)
- [🌐 Problemas de AJAX](#problemas-de-ajax)
- [📝 Problemas con Formularios](#problemas-con-formularios)
- [⚡ Problemas de Rendimiento](#problemas-de-rendimiento)
- [🔧 Problemas con Assets](#problemas-con-assets)
- [🐛 Debug y Logs](#debug-y-logs)

## 🚀 Problemas de Instalación

### Error: "Class not found" al usar componentes de Glory

**Síntomas:** Aparece un error `Fatal error: Class 'Glory\Components\...' not found`
...
2. **Verificar estructura de archivos:**
   Asegúrate de que la carpeta `Glory/` esté en la raíz del tema y contenga:
   ```
   Glory/
   ├── src/
   ├── Components/
...
3. **Verificar namespace en tu código:**
   ```php
   // Correcto
...
**Causa:** El autoload de Composer no está configurado correctamente.
...
**Síntomas:** No se pueden escribir archivos de cache o configuración.
...
1. **JavaScript no cargado:**
   Verifica que `glory-modal.js` esté en cola:
   ```php
...
3. **Conflicto de IDs:**
   Asegúrate de que cada modal tenga un ID único.
...
**Causa común:** Estructura de datos incorrecta.
...
### Navegación AJAX no funciona

**Verificaciones:**

1. **JavaScript cargado:**
   Verifica que `glory-ajax-nav.js` esté incluido.
...
1. **Campos requeridos sin validar:**
   ```php
   // Asegurar que 'requerido' esté en true
   FormBuilder::campoTexto([
       'nombre' => 'email',
       'label' => 'Email',
       'requerido' => true, // ← Importante
       'tipo' => 'email'
   ]);
   ```
...
### Los formularios se envían pero no procesan

**Debug:**

1. **Verificar acción del botón:**
   ```php
...
3. **Optimización de imágenes:**
   ```php
   // Usar GloryImage para optimización automática
...
2. **Ruta del archivo correcta:**
   ```php
   // Ruta relativa desde la raíz del tema
   // Si el archivo está en: wp-content/themes/mi-tema/assets/css/estilo.css
   // La ruta debe ser: 'assets/css/estilo.css'
   ```

3. **Archivo existe:**
   ```bash
   # Verificar desde la raíz del tema
...
AssetManager::define('script', 'mi-script', 'assets/js/script.js', [
    'deps' => ['jquery', 'glory-core'], // Cargar después de estos
...
1. **Revisa la documentación específica** del componente que estás usando
2. **Busca en los logs** de Glory por mensajes de error
3. **Verifica compatibilidad** con tu versión de WordPress/PHP
4. **Comenta en el repositorio** con detalles específicos del problema

Recuerda: la mayoría de los problemas tienen soluciones documentadas. ¡La paciencia y el método sistemático son tus mejores aliados!
