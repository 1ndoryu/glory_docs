...
description: Manejador de sincronización de posts individuales con comparación inteligente de cambios
---
...
El `PostSyncHandler` es el componente central para la gestión individual de posts durante procesos de sincronización. Maneja la creación, actualización y comparación de posts, integrando servicios de relaciones y integridad de medios para mantener la consistencia del contenido.

## Funcionalidades Principales

### 📝 Operaciones CRUD Básicas
- **Creación de posts**: Inserción con datos preparados y relaciones
- **Actualización inteligente**: Modificación basada en comparación de cambios
- **Comparación de estados**: Detección automática de diferencias

### 🔍 Sistema de Comparación
- **Campos principales**: Título, contenido, estado, extracto
- **Metadatos personalizados**: Comparación de valores meta
- **Relaciones externas**: Imágenes, galerías, taxonomías

### 🔗 Integración de Servicios
- **PostRelationHandler**: Gestión de relaciones del post
- **MediaIntegrityService**: Verificación de medios asociados
...
$handler->update($postId, [
    'titulo' => 'Página de Contacto Actualizada',
    'contenido' => 'Nuevo contenido...',
    'estado' => 'publish'
], false); // false = actualización normal, true = forzada
```

### Comparación de Cambios

```php
// Verificar si un post necesita actualización
...
class PostSyncHandler
{
    ...
    // Constructor inyecta dependencias automáticamente
...
public function create(string $postType, array $definition): ?int
{
    ...
    if (is_wp_error($postId)) {
        GloryLogger::error("FALLÓ al insertar post", [
            'error' => $postId->get_error_message()
        ]);
        return null;
    }

    // 3. Establecer relaciones (imágenes, taxonomías, etc.)
    $this->relationHandler = new PostRelationHandler($postId);
...
// Fuerza actualización completa, limpia metadatos antiguos
```
...
public function update(int $postId, array $definition, bool $isForced): void
{
    ...
    if (is_wp_error($result)) {
        GloryLogger::error("FALLÓ al actualizar post ID {$postId}");
        return;
    }
...
public function needsUpdate(\WP_Post $postDb, array $definition): bool
{
    ...
    // Comparar metadatos (excluyendo taxonomías)
...
### Comparación de Campos Principales
...
private function compareMetaFields(\WP_Post $post, array $definition): bool
{
    ...
    foreach ($definedMeta as $key => $value) {
        // Saltar taxonomías (se manejan en relaciones)
...
private function compareThumbnail(\WP_Post $post, array $definition): bool
{
    ...
    if (empty($currentThumbId)) {
        // Solo marcar como cambio si existe adjunto válido
...
private function cleanupMeta(int $postId, array $definition): void
{
    ...
    foreach (array_keys($existingMeta) as $existingKey) {
        ...
        // Eliminar metadatos no definidos en la nueva definición
...
Actualiza un post existente con opción de forzamiento.

### `needsUpdate(\WP_Post $postDb, array $definition): bool`
Determina si un post necesita ser actualizado comparando con su definición.

## Ejemplos Prácticos

### Sincronización de Página de Inicio
...
### Actualización Forzada de Contenido

```php
// Para actualizar plantillas o contenido crítico
...
### Verificación Masiva de Cambios
...
if ($postId === null) {
    ...
    GloryLogger::error("Fallo en creación de post", [
...
```php
// Solo sincronizar si han pasado X días desde la última edición
    ...
    if (!empty($definition['metaEntrada'])) {
        foreach ($definition['metaEntrada'] as $key => $value) {
            if (taxonomy_exists($key)) {
                // Validar términos existen o se pueden crear
            }
        }
    }
...
class PostSyncHandlerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testNeedsUpdate()
    {
        ...
        $definition = ['titulo' => 'Nuevo Título'];
...
// Verificar logs durante sincronización
function debugSincronizacion($postId, $definition) {
    GloryLogger::info("DEBUG: Iniciando sincronización", [
        'postId' => $postId,
        'definition' => $definition,
