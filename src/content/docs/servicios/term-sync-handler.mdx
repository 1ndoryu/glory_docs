...
description: Manejador de sincronización de términos de taxonomía con gestión automática de categorías
---
...
El `TermSyncHandler` gestiona la sincronización automática de términos de taxonomía, especialmente categorías, durante los procesos de sincronización de contenido por defecto. Mantiene la consistencia entre las definiciones de términos y los términos existentes en WordPress.

## Funcionalidades Principales

### 📂 Gestión de Categorías
- **Sincronización automática**: Crea y actualiza categorías desde definiciones
- **Eliminación de obsoletos**: Remueve términos que ya no están en las definiciones
- **Preservación de datos**: Mantiene relaciones existentes cuando es posible

### 🏷️ Control de Términos
- **Creación condicional**: Solo crea términos inexistentes
- **Actualización de metadatos**: Gestiona campos adicionales como descripciones
- **Imágenes de categoría**: Asocia imágenes a términos usando AssetsUtility

### 🔄 Ciclo de Vida
- **Sincronización completa**: Ejecuta creación, actualización y limpieza en secuencia
...
// Sincronizar todas las definiciones de términos
$handler->sync();
```
...
$GLOBALS['glory_categorias_definidas'] = [
    [
        'nombre' => 'Noticias',
        'descripcion' => 'Artículos y novedades de la empresa',
        'imagenAsset' => 'glory::news-category.jpg'
    ],
    [
        'nombre' => 'Productos',
        'descripcion' => 'Información sobre nuestros productos',
...
public function sync(): void
{
    ...
    // 1. Eliminar términos obsoletos
    $this->deleteObsoleteTerms($categoriasDef);

    // 2. Crear/actualizar términos definidos
    $this->createOrUpdateTerms($categoriasDef);
}
```

## Gestión de Términos Obsoletos
...
{
    // Obtener nombres de términos definidos
    $definedNames = array_filter(array_column($definitions, 'nombre'));

    // Buscar términos gestionados automáticamente
    ...
    if (is_wp_error($managedTerms)) {
        return; // Error consultando términos
    }

    // Eliminar términos no definidos
...
### Marcado de Términos Gestionados

```php
// Constante para identificar términos gestionados
const META_CLAVE_CATEGORIA_GESTIONADA = '_glory_managed_category';

// Al crear/actualizar términos
update_term_meta($term->term_id, self::META_CLAVE_CATEGORIA_GESTIONADA, '1');
```

## Creación y Actualización de Términos

### Proceso de Gestión Individual
...
private function createOrUpdateTerms(array $definitions): void
{
    foreach ($definitions as $def) {
        ...
        // Si el término existe, actualizarlo
...
## Gestión de Imágenes de Categoría
...
### `sync(): void`
Sincroniza todas las definiciones de términos globales.

## Consideraciones de Seguridad

### Taxonomía Limitada

Actualmente solo gestiona la taxonomía `category` (categorías por defecto de WordPress). Para otras taxonomías personalizadas, se requeriría extensión del código.

### Preservación de Datos

- **No elimina términos con posts**: Los términos que tienen posts asociados no se eliminan automáticamente
- **No sobrescribe imágenes**: Si un término ya tiene imagen asignada, no se reemplaza
- **Mantiene slugs**: Los slugs de términos existentes se preservan
...
```php
// Logging se puede activar descomentando líneas en el código:
/*
GloryLogger::info("TermSyncHandler: Categoría '{$term->name}' eliminada.");
GloryLogger::info("TermSyncHandler: Categoría '{$nombre}' creada.");
GloryLogger::info("TermSyncHandler: Categoría '{$term->name}' actualizada.");
*/
```
...
### Definición Básica de Categorías
...
function definirCategoriasEmpresa() {
    $GLOBALS['glory_categorias_definidas'] = [
        [
            'nombre' => 'Empresa',
            'descripcion' => 'Información institucional y corporativa',
        ...
        [
            'nombre' => 'Blog',
            'descripcion' => 'Artículos, noticias y actualidad',
            'imagenAsset' => 'elements::blog-icon.jpg'
        ],
        [
            'nombre' => 'Soporte',
            'descripcion' => 'Preguntas frecuentes y soporte técnico',
            // Sin imagen específica
        ]
    ];
}
...
```php
// Sincronización automática al activar tema
...
// Sincronización manual vía AJAX
function ajax_sync_terms() {
    ...
    wp_send_json_success(['message' => 'Términos sincronizados']);
}
...
function verificarSincronizacionTerminos(): array {
    ...
    foreach ($definiciones as $def) {
        ...
        if (!$term) {
            $problemas[] = "Término '{$def['nombre']}' no existe";
            continue;
        }

        // Verificar descripción
        if ($term->description !== ($def['descripcion'] ?? '')) {
            $problemas[] = "Descripción de '{$def['nombre']}' no coincide";
        }
...
## Extensión para Otras Taxonomías

### Soporte para Taxonomías Personalizadas

```php
// Versión extendida para múltiples taxonomías
...
class DefaultContentSynchronizer
{
    ...
    public function sincronizar(): void {
        ...
        // 2. Luego sincronizar posts (que pueden depender de términos)
...
```php
// Sincronización automática en eventos importantes
add_action('init', function() {
    // Verificar si es necesario sincronizar (una vez por día)
...
// Sincronización manual vía admin
...
if (count($names) !== count(array_unique($names))) {
    error_log('Nombres de términos duplicados');
}
```

### Imágenes No Se Asocian

```php
// Verificar que AssetsUtility esté disponible
...
if ($term) {
    ...
    if ($postsCount > 0) {
        error_log("No se puede eliminar término '{$term->name}': tiene {$postsCount} posts");
    }
}
```
...
class TermSyncHandlerTest extends \PHPUnit\Framework\TestCase
{
    public function testSyncCreatesTerms()
    {
        ...
        $GLOBALS['glory_categorias_definidas'] = [
            ['nombre' => 'Test Category', 'descripcion' => 'Descripción de prueba']
        ];
        ...
        $this->assertEquals('Descripción de prueba', $term->description);
    }

    public function testSyncDeletesObsoleteTerms()
    {
        // Crear término manualmente
        wp_insert_term('Term To Delete', 'category');

        // Definir solo otro término
        $GLOBALS['glory_categorias_definidas'] = [
        ...
        // Verificar que se eliminó el término obsoleto
...
```php
// Cache de términos para evitar consultas repetidas
