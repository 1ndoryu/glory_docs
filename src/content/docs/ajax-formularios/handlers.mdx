...
description: Guía completa para crear y gestionar handlers de formularios en Glory Framework
sidebar:
  label: Handlers
  order: 3
---
...
# 🎯 Handlers de Formulario - Procesamiento del Servidor

Los **Handlers de Formulario** son el corazón del procesamiento del lado servidor en Glory Framework. Cada handler es una clase especializada que valida, procesa y responde a formularios específicos, proporcionando una arquitectura modular y segura.

## 📋 Conceptos Básicos

### ¿Qué es un Handler?

Un handler es una clase PHP que implementa la interfaz `FormHandlerInterface` y contiene la lógica específica para procesar un tipo determinado de formulario.

**Características principales:**
- ✅ **Validación automática** de datos
- ✅ **Sanitización integrada** de entradas
...
```
FormHandler (Router)
├── Busca handler por subAccion
├── Instancia la clase apropiada
├── Ejecuta método procesar()
└── Formatea respuesta JSON

Handler Específico
├── Valida datos de entrada
├── Ejecuta lógica de negocio
├── Interactúa con BD/API
├── Maneja archivos si existen
└── Retorna respuesta estructurada
```

## 🚀 Handlers Incluidos en Glory
...
**Campos opcionales:**
- `postTitle`: Título personalizado
...
$form->addHidden('subAccion', 'ActualizarPublicacion')
     ...
     ->addText('postTitle', 'Nuevo título');
```

#### `EliminarPublicacionHandler`

Elimina publicaciones con verificación de permisos.

```php
$form->addHidden('subAccion', 'EliminarPublicacion')
     ->addHidden('postId', $postId)
     ->addCheckbox('confirmar', 'Confirmar eliminación');
```
...
Actualiza información de perfil de usuario.

```php
$form->addHidden('subAccion', 'ActualizarPerfil')
     ->addText('first_name', 'Nombre')
     ->addText('last_name', 'Apellido')
     ->addTextarea('description', 'Biografía')
     ->addFile('avatar', 'Foto de perfil');
```

#### `CambiarPasswordHandler`

Cambia contraseña con verificación de contraseña actual.

```php
$form->addHidden('subAccion', 'CambiarPassword')
     ->addPassword('password_actual', 'Contraseña actual')
     ->addPassword('password_nuevo', 'Nueva contraseña')
     ->addPassword('password_confirmar', 'Confirmar contraseña');
```
...
$form->addHidden('subAccion', 'GuardarMeta')
     ...
     ->addSelect('meta_categoria', 'Categoría', $opciones)
     ->addCheckbox('meta_destacado', 'Producto destacado');
```

#### `GuardarOpcionesHandler`

Guarda opciones de configuración.

```php
$form->addHidden('subAccion', 'GuardarOpciones')
     ->addText('opcion_sitio_titulo', 'Título del sitio')
...
Envía newsletters a listas de suscriptores.
...
## 🛠️ Creación de Handlers Personalizados
...
class MiFormularioHandler implements FormHandlerInterface
{
    public function procesar(array $postDatos, array $archivos): array
    {
        try {
            ...
            // Re-lanzar para manejo automático
            throw $e;
        }
    }
...
return [
    ...
    'reload' => true,                   // Recargar página
    'refresh' => '#contenedor',         // Recargar elemento específico

    // Mensajes adicionales
    'messages' => [
        'success' => ['Usuario creado', 'Email enviado'],
        'info' => ['Se activará en 24 horas']
    ]
];
```
...
```php
// El sistema captura automáticamente las excepciones
...
public function procesar(array $postDatos, array $archivos): array
{
    // Estos campos ya están sanitizados:
    // - Texto: sanitize_text_field()
    // - Email: sanitize_email()
    // - Textarea: sanitize_textarea_field()
    // - Número: intval() o floatval()
...
{
    // Lógica pesada aquí (envío de emails, procesamiento de pagos, etc.)
...
class MaquinaEstadosHandler implements FormHandlerInterface
{
    ...
    public function procesar(array $postDatos, array $archivos): array
    {
        ...
        if (!in_array($accion, $this->estados[$estadoActual])) {
            throw new \Exception("Transición '{$accion}' no permitida desde estado '{$estadoActual}'");
        }
...
class MiHandler implements FormHandlerInterface
{
    public function procesar(array $postDatos, array $archivos): array
    {
        ...
        // 3. Sanitización adicional si es necesaria
    ...
    // Métodos privados organizados por responsabilidad
...
Los handlers proporcionan una arquitectura sólida y flexible para procesar formularios de manera segura y mantenible. Siguiendo estas mejores prácticas, puedes crear handlers robustos que se integren perfectamente con el ecosistema de Glory Framework.
