...
description: "Definir y registrar CPTs (Custom Post Types) desde código"
---

`PostTypeManager` ofrece una API declarativa para crear CPTs con validación, etiquetas automáticas en español y metadatos por defecto.

## Resumen
- **Para qué sirve**: definir CPTs versionables con defaults sensatos.
- **Dónde va**: en el tema (`/App`), por ejemplo `App/Content/postType.php`.
...
- Si pasas `$metaDefault`, se añade `custom-fields` a `supports` si hiciera falta.

### `$singular`/`$plural`
- Si ambos existen y no defines `labels`, se generan etiquetas completas en español (dominio `glory`).

### `$metaDefault`
- Metadatos que se añaden automáticamente al crear una nueva entrada del CPT (no en updates).
...
- Enganche automático al `save_post_{slug}` para entradas nuevas.
...
## Integración con DefaultContentManager
...
## Inicialización desde el tema
- Define tus CPTs en `App/Content/postType.php`.
- El core registra los managers si las features están activas en `App/Config/control.php`.
- Si cambias slugs/reescrituras, ejecuta `PostTypeManager::limpiarReglasReescritura()` una vez (por ejemplo, en activación del tema/plugin).

## Errores frecuentes
- Slug inválido o reservado.
