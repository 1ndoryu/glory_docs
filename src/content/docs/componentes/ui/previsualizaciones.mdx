...
description: Gestión interactiva de archivos con drag & drop, selección por clic y previsualización en tiempo real
---
...
Sistema avanzado para la gestión interactiva de archivos que proporciona previsualización en tiempo real, soporte para drag & drop y selección por clic.
...
- **Imágenes** (`.previewImagen`, `.preview`): JPG, PNG, GIF, WebP, SVG
- **Audio** (`.previewAudio`): MP3, WAV, OGG, M4A
- **Archivos genéricos** (`.previewFile`): Cualquier tipo de archivo

### Métodos de Interacción
- **Clic en botones**: `.botonPreviewImagen`, `.botonPreviewAudio`, `.botonPreviewFile`
- **Clic en áreas de preview**: Zonas interactivas que abren el selector
...
Vincula manualmente inputs y previews cuando no están en la misma jerarquía.
...
<div class="previewContenedor" data-extrapreview=".elemento-extra">
    <div class="elemento-extra oculto">Suelta el archivo aquí</div>
    <!-- Contenido normal -->
</div>
```

### data-extrapreview-on
Controla cuándo mostrar el preview extra: `"click"`, `"drag"`, `"drop"`, `"always"`.
...
Muestra una imagen desde una URL (útil para imágenes existentes).
...
- `.previewRemover`: Botón de eliminar imagen (se crea automáticamente)

### Campos Ocultos Automáticos
El sistema gestiona automáticamente campos hidden para WordPress:
- `.glory-image-id`: ID de la imagen en WordPress
- `.glory-image-url`: URL de la imagen
- `input[name$="_delete"]`: Indicador de eliminación

## 🔄 Flujo de Funcionamiento

### 1. Inicialización
- Se vinculan todos los event listeners
- Se previene duplicación en recargas (`gloryRecarga`)
...
### 4. Eliminación
- Botón X automático para imágenes
...
.previewImagen { /* Área de preview de imagen */ }
.previewAudio { /* Área de preview de audio */ }
.previewFile { /* Área de preview de archivo */ }
.botonPreviewImagen { /* Botón disparador */ }
...
### Campos Esperados
```php
// El sistema espera estos campos para integración completa
...
4. **Imágenes no se eliminan**: Revisar campos hidden y placeholders
...
- **Prevención de Duplicados**: Sistema de flags para evitar listeners múltiples
- **Optimización**: FileReader solo para archivos válidos
- **Memory Management**: Limpieza automática de elementos y listeners

## 🔄 Compatibilidad

- ✅ **Modern Browsers**: ES6+, File API, Drag & Drop API
- ✅ **Mobile**: Touch events y responsive design
- ✅ **WordPress**: Integración completa con media library
- ✅ **Accesibilidad**: Soporte básico para navegación por teclado
- ✅ **Progressive Enhancement**: Funciona sin JavaScript habilitado (solo input básico)
