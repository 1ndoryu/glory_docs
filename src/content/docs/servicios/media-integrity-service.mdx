...
description: Servicio de integridad automática de medios con reparación inteligente de imágenes faltantes
---
...
El `MediaIntegrityService` es un servicio especializado que mantiene la integridad de los medios asociados a posts de WordPress. Automáticamente detecta y repara imágenes destacadas faltantes, galerías rotas y URLs corruptas en contenido y metadatos.

## Funcionalidades Principales

### 🔍 Verificación Automática
- **Imágenes destacadas**: Detecta thumbnails faltantes o corruptos
- **Galerías de medios**: Verifica integridad de colecciones de imágenes
- **URLs rotas**: Identifica enlaces a archivos inexistentes

### 🔧 Reparación Inteligente
- **Reasignación automática**: Usa assets existentes para reemplazar imágenes faltantes
- **Fallbacks determinísticos**: Asigna imágenes por defecto de forma consistente
- **Sanitización de contenido**: Corrige URLs rotas en posts y metadatos

### ⏰ Gestión Temporal
- **Ventana anti-reintento**: Evita asignaciones repetidas fallidas
- **Cooldown inteligente**: Espera períodos apropiados entre intentos

## Uso Básico

### Reparación Completa de Medios
...
1. **Verificación inicial**: Comprueba si existe thumbnail asignado
2. **Validación de archivo**: Confirma que el archivo físico existe
3. **Búsqueda de asset alternativo**: Localiza asset válido usando referencia
4. **Asignación de fallback**: Usa imagen por defecto si no hay alternativa
...
```php
// Si no hay galería definida, crea una automática
private function chooseFallbackGalleryForPost(int $postId, int $count): array
{
    // Retorna array de assets para galería
...
// Formatos soportados
$referenciasValidas = [
    'glory::imagen.jpg',        // Alias específico
...
```php
// Verifica si un asset existe físicamente
...
## Integración con DefaultContentSynchronizer

### Sincronización Automática

```php
// El servicio se ejecuta automáticamente durante la sincronización
...
public function restablecer()
{
    ...
    // Reparación post-restablecimiento
    $this->repairMediaIntegrityForAll();
}
```
...
Repara todos los medios asociados a un post específico.

### `repairFeaturedImage(int $postId, ?string $fallbackAssetRef): void`
Repara específicamente la imagen destacada.

### `repairGallery(int $postId, array $fallbackAssets): void`
Repara la galería de medios del post.
...
```php
// Fallbacks específicos por post type
$fallbacksPorTipo = [
...
// Logging de fallos
GloryLogger::warning("MediaIntegrity: Fallback falló", [
    'asset' => $fallbackAsset,
    'postId' => $postId,
...
function repararMediosPost($postId) {
    ...
    // Obtener definición del post (si existe)
...
```php
// Hook para verificación periódica
...
function limpiarMetadatosRotos($postId) {
    ...
    // Fuerza sanitización de metadatos
...
```php
// Para grandes volúmenes de posts
$postsPorLote = 100;
$loteActual = 0;
...
- **Caché de assets**: AssetsUtility mantiene caché interno
- **Verificación condicional**: Solo repara cuando es necesario
- **Índices de BD**: Usa índices optimizados de WordPress

## Troubleshooting

### Imágenes No Se Reparan
...
if (!$estructuraValida) {
    // Configuración de uploads corrupta
}
```

## Casos de Uso Avanzados

### Integración con Plugins de Medios

```php
// Hook para plugins de optimización de imágenes
add_filter('wp_generate_attachment_metadata', function($metadata, $attachmentId) {
    // Después de optimización, verificar integridad
    $service = new MediaIntegrityService();
...
```php
// Backup automático antes de reparaciones
