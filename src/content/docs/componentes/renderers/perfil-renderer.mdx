...
description: Componente para renderizar perfiles de usuario con imagen, información y acciones
sidebar:
...
El **PerfilRenderer** es un componente especializado para renderizar información de perfiles de usuario en WordPress. Proporciona una interfaz unificada para mostrar avatares, datos personales, estadísticas y acciones relacionadas con usuarios, tanto del usuario actual como de otros usuarios del sistema.

## Filosofía

PerfilRenderer está diseñado para ser:
- **Flexible**: Adaptable a diferentes contextos y necesidades de visualización
- **Completo**: Incluye imagen, datos y acciones en un solo componente
- **Seguro**: Respeta permisos y privacidad de usuarios
- **Extensible**: Fácil de personalizar y expandir

## Características Principales

- **Avatar inteligente**: Soporte para imagen personalizada o Gravatar
- **Información contextual**: Datos adaptados según permisos del usuario
- **Acciones dinámicas**: Botones y enlaces basados en relación con el usuario
- **Estilos responsivos**: Adaptable a diferentes tamaños de pantalla
- **Cache automático**: Optimización de rendimiento con cache de datos
...
#### Modo Estándar
Avatar, nombre y rol básico.

#### Modo Completo
Información detallada con estadísticas y acciones.

#### Modo Editable
Vista especial para edición de perfil propio.
...
// O renderizar con configuración específica
PerfilRenderer::render([
    'modo' => 'completo',
...
```php
// Renderizar perfil de usuario específico
$userId = 123;
...
class AvatarHandler
{
    public static function obtenerAvatar($userId, $config = [])
    {
        ...
        // 2. Usar Gravatar si está habilitado
...
```php
// Datos a mostrar según permisos
...
```php
// Acciones según relación con el usuario
...
```php
// Verificar permisos antes de mostrar información sensible
function puedeVerInformacion($userIdSolicitado, $currentUserId, $esAdmin = false)
{
    // El propio usuario siempre puede ver su información
    if ($userIdSolicitado === $currentUserId) {
        return true;
    }

    // Los administradores pueden ver toda la información
    if ($esAdmin) {
        return true;
    }
    ...
    // Verificar relación (amigos, seguidores, etc.)
...
function debug_avatar_usuario($userId)
{
    ...
    echo "<!-- Gravatar existe: " . ($gravatarExiste ? 'SÍ' : 'NO') . " -->\n";
}
```

### Información incorrecta se muestra
...
function diagnosticar_problemas_perfil($userId, $currentUserId)
{
    ...
    echo "<!-- Es propio perfil: " . ($esPropioPerfil ? 'SÍ' : 'NO') . " -->\n";
...
**Problema:** Los botones de acción no responden
**Causa:** JavaScript no está cargado o hay conflictos
**Solución:**
```javascript
// Verificar que el JavaScript esté funcionando
document.addEventListener('DOMContentLoaded', function() {
    ...
    acciones.forEach(function(accion, index) {
        console.log('Acción ' + index + ':', accion.href || accion.onclick);
    });
});
```
...
<div class="perfil-usuario perfil-modo-<?php echo esc_attr($config['modo']); ?> <?php echo esc_attr($config['clasesCss']); ?>">
    <div class="perfil-avatar">
        <?php echo $avatarHtml; ?>
        <?php if ($estadoOnline): ?>
            <span class="perfil-estado-online" title="En línea"></span>
        <?php endif; ?>
    </div>
...
<Aside type="caution">
⚠️ **Importante**: Siempre verifica permisos antes de mostrar información sensible de usuarios. Nunca muestres emails, teléfonos o direcciones sin el consentimiento explícito del usuario o permisos administrativos.
</Aside>

<Aside type="tip">
💡 **Tip**: Para aplicaciones complejas, considera implementar un sistema de "amistades" o "seguidores" para controlar qué información de perfil pueden ver otros usuarios.
</Aside>
