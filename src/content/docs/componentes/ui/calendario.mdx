...
description: Calendario interactivo con navegación mensual, selección de fechas y gestión de contexto
---
...
Calendario interactivo agnóstico que proporciona selección de fechas con navegación mensual, marcado de fechas especiales y gestión contextual de selección.
...
- **Navegación mensual**: Botones anterior/siguiente mes
- **Selección visual**: Clic en días para seleccionar fechas
- **Marcado automático**: Día actual y fecha seleccionada destacados
- **Posicionamiento dinámico**: Aparece junto al elemento disparador
- **Cierre automático**: Click fuera para cerrar
- **Manejo contextual**: Soporte para diferentes tipos de fecha

### Elementos de Interfaz
- **Navegación**: Mes/Año actual con botones prev/next
- **Días de semana**: Cabecera con nombres de días
- **Días del mes**: Celdas interactivas con números
...
<div id="calCont" class="calCont" style="display: none; position: absolute; z-index: 1001;">
    <!-- Navegación -->
    <div class="calNav">
        <button id="calPrev" class="calNavBtn">‹</button>
        <span id="calMesAnio" class="calMesAnio">Enero 2024</span>
        <button id="calNext" class="calNavBtn">›</button>
    </div>
    ...
    <table class="calTabla">
        ...
        <tbody id="calBody" class="calBody">
            <!-- Filas de días generadas dinámicamente -->
        </tbody>
    </table>
...
<button class="mi-calendario">Fecha Límite</button>
```
...
#### data-tipofecha
Tipo de fecha (para contextos específicos).
...
Muestra el calendario junto a un elemento específico.
...
- **`.calNav`**: Barra de navegación mes/año
- **`.calNavBtn`**: Botones anterior/siguiente
- **`.calMesAnio`**: Display de mes y año actual
- **`.calTabla`**: Tabla del calendario
- **`.calBody`**: Cuerpo con los días
- **`.calAcciones`**: Contenedor de botones de acción
- **`.calBtnAccion`**: Botones Hoy/Borrar

### Clases de Días
- **`.calDia`**: Celda individual de día
- **`.calDiaNum`**: Número del día
- **`.calDiaHoy`**: Día actual (hoy)
- **`.calDiaSel`**: Día seleccionado
- **`.calDiaFuera`**: Días fuera del mes actual
...
function onSelect(fechaISO, contexto) {
    ...
    // contexto: objeto con información contextual
...
```javascript
{
    elementoDisparador: HTMLElement,    // Elemento que abrió el calendario
    tipoFecha: 'limite',               // Tipo de fecha
    fechaLimiteValor: string,          // Valor actual
    esParaTareaEspecifica: boolean,    // Para usos específicos
...
```javascript
// Para contextos específicos (ej: tareas)
...
- **Click en día**: Selecciona fecha y cierra
- **Click en navegación**: Cambia mes
- **Click en "Hoy"**: Selecciona fecha actual
- **Click en "Borrar"**: Limpia selección
...
document.addEventListener('gloryRecarga', function() {
    // Los handlers se mantienen automáticamente
});
```
...
4. **Callback no funciona**: Revisar sintaxis de la función `onSelect`
...
- **Inicialización lazy**: Solo crea estructura cuando es necesario
- **Reutilización de DOM**: Mantiene elementos entre aperturas
- **Event delegation**: Un solo listener global para clicks
- **Memory safe**: Limpieza automática de referencias

## 🔄 Compatibilidad

- ✅ **Modern Browsers**: ES6+, Date API, DOM manipulation
- ✅ **Legacy**: Funciona con polyfills para métodos modernos
...
- ✅ **Internationalization**: Nombres de meses en español (configurable)
