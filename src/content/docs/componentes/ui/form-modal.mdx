...
description: Gestor avanzado de formularios dentro de modales con creación/edición AJAX, upload de imágenes y validaciones dinámicas
---
...
El sistema de formularios modales proporciona una solución completa para gestionar formularios dentro de modales con funcionalidades avanzadas como creación/edición AJAX, upload de imágenes con `wp.media`, selects dependientes, validaciones dinámicas y gestión de estado compleja.

## 🚀 Inicio Rápido

```html
<!-- Trigger para abrir modal en modo creación -->
<button type="button"
        class="abrir-modal"
...
<!-- Trigger para abrir modal en modo edición -->
<button type="button"
        class="abrir-modal"
        data-modal-target="mi-modal"
        data-form-mode="edit"
...
#### Modo Creación (`data-form-mode="create"`)
- Limpia automáticamente todos los campos del formulario
- Resetea uploads de imágenes a estado inicial
- Elimina cualquier dato residual de ediciones previas
- Prepara el formulario para nuevos datos

#### Modo Edición (`data-form-mode="edit"`)
- Carga datos vía AJAX usando `data-fetch-action`
- Rellena automáticamente todos los campos del formulario
- Gestiona imágenes existentes y previews
- Mantiene estado consistente entre cargas

### Sistema de Upload de Imágenes
Integración completa con `wp.media` para gestión de imágenes:

```html
<!-- Uploader básico de imágenes -->
...
Sistema declarativo para cargar opciones dinámicamente:
...
<select name="ciudad"
        ...
        data-fm-placeholder-deshabilitado="Seleccione un país primero">
    <option value="">Seleccione ciudad</option>
</select>
```

### Validación Dinámica de Submit
Habilita/deshabilita el botón de envío basado en campos requeridos:
...
### Gestión de Servicios/Grupos
Sistema especial para checkboxes con opción "Seleccionar todos":
...
- **Creación**: Campos limpios, placeholders activos, botones de eliminación ocultos
- **Edición**: Campos poblados, imágenes cargadas, previews visibles
- **Validación**: Botón submit habilitado/deshabilidado dinámicamente

### Gestión de Imágenes
- **Preview interactivo**: Click en preview abre selector de imágenes
- **Estados visuales**: Placeholder cuando vacío, imagen cuando cargada
- **Botón eliminar**: Visible solo cuando hay imagen seleccionada
- **Sincronización**: ID y URL mantenidos en campos ocultos

### Estructura HTML Generada

```html
<!-- Modal con formulario típico -->
        ...
        <form class="gloryForm" data-object-id="123">
            ...
            <div class="glory-image-uploader">
                ...
                <div class="image-preview">
                    ...
                    <button class="previewRemover">×</button>
                </div>
...
- `data-modal-title`: Título base del modal
- `data-modal-title-create`: Título específico para modo creación
- `data-modal-title-edit`: Título específico para modo edición

#### Modo Creación
- `data-form-mode="create"`: Activa modo creación
- `data-submit-action`: Action AJAX para guardar
- `data-submit-text`: Texto del botón submit

#### Modo Edición
- `data-form-mode="edit"`: Activa modo edición
...
- `data-fm-keep-current`: Mantiene valor actual visible aunque no esté en opciones
- `data-fm-exclude-param`: Parámetro para excluir opción actual en edición
...
#### `gloryFormModal()`
Función principal que inicializa el sistema de formularios modales.

```javascript
// Se ejecuta automáticamente en DOMContentLoaded y gloryRecarga
gloryFormModal();
```
...
Antes de preparar el formulario para creación.

```javascript
document.addEventListener('gloryFormModal:beforeCreate', function(event) {
    // Preparar datos específicos antes de limpiar
});
```

#### `gloryFormModal:afterCreate`
Después de preparar el formulario para creación.

```javascript
document.addEventListener('gloryFormModal:afterCreate', function(event) {
    // Configuraciones adicionales post-creación
});
```

#### `gloryFormModal:beforeEdit`
Antes de cargar datos para edición.
...
Después de cargar datos para edición.
...
```javascript
// Función interna para llamadas AJAX
...
### Gestión de Productos con Dependencias
```html
<form class="gloryForm">
    ...
    <select name="subcategoria"
            ...
            data-fm-placeholder-deshabilitado="Seleccione una categoría primero">
...
<form class="gloryForm" data-fm-submit-habilitar-cuando="titulo">
    <input type="text" name="titulo" placeholder="Título requerido" required>
    ...
    <button type="submit" class="dataSubir" disabled>Guardar Configuración</button>
</form>
```

## 🔄 Compatibilidad

- ✅ **WordPress**: Integración completa con `wp.media` para uploads
- ✅ **AJAX**: Compatible con `gloryAjax` y fallback manual
- ✅ **Modales**: Funciona con cualquier sistema de modales que emita `gloryModal:open`
- ✅ **Responsive**: Se adapta a diferentes tamaños de pantalla
- ✅ **Accesibilidad**: Soporte básico para navegación por teclado
- ✅ **Validación**: Validación HTML5 nativa + validación dinámica
- ✅ **Estados**: Gestión robusta de estados creación/edición
- ✅ **Dependencias**: Sistema de selects dependientes multi-nivel
...
2. **Datos no se cargan en edición**: Revisar `data-fetch-action` y `data-object-id`
3. **Selects dependientes no cargan**: Verificar `data-fm-depende` y `data-fm-accion-opciones`
4. **Imágenes no se suben**: Confirmar que `wp.media` está disponible
5. **Botón submit permanece deshabilitado**: Revisar `data-fm-submit-habilitar-cuando`
...
```javascript
// Verificar configuración de uploader
...
### Reset Manual
```javascript
// Forzar reinicialización (solo para debug)
