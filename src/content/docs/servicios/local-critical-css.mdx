...
description: Servicio de generación local de CSS crítico usando Node.js y Puppeteer
sidebar:
...
- **Renderizado real**: Usa Puppeteer para renderizar páginas como un navegador real
- **Análisis inteligente**: Extrae solo el CSS necesario para el contenido visible
...
graph TD
    ...
    B --> C[Inyectar parámetro noAjax]
    C --> D[Renderizar página]
    D --> E[Esperar renderWait ms]
    E --> F[Extraer CSS crítico con Penthouse]
...
// Para páginas complejas (e-commerce, etc.)
$complejaConfig = [
    'timeout' => 120000,     // 2 minutos
...
```php
// El GestorCssCritico ya implementa cache automático
...
class CssCriticoInteligente
{
    ...
    {
        // No generar para páginas administrativas
...
class BuildProcessCssCritico
{
    public static function generarCssParaDespliegue()
    {
        $urls = [
            ...
            home_url('/acerca-de/'),
            home_url('/contacto/'),
            // Agregar más URLs según necesidad
        ];
...
class MonitorCssCritico
{
    ...
    public static function registrarGeneracion(string $url, int $bytes)
    {
        ...
        // Mantener solo las últimas 1000 generaciones
...
- Contenido dinámico no renderizado
- Errores de JavaScript bloquean renderizado

**Diagnóstico:**
```php
// Verificar que la página sea accesible
