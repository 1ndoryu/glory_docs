...
El **SchedulerRenderer** es un componente avanzado de Glory Framework diseñado para renderizar programadores y calendarios de citas con una interfaz visual intuitiva. Es ideal para sistemas de reservas, agendas médicas, barberías, salones de belleza y cualquier aplicación que requiera gestión visual de horarios.

## Filosofía

SchedulerRenderer está diseñado para ser:
- **Visual**: Interfaz clara y fácil de entender
- **Flexible**: Adaptable a diferentes tipos de recursos y horarios
- **Interactivo**: Soporte para eventos y acciones del usuario
- **Escalable**: Manejo eficiente de múltiples recursos y eventos

## Características Principales

- **Vista de cuadrícula**: Muestra tiempo vs recursos en formato tabular
- **Eventos visuales**: Representación gráfica de citas y reservas
- **Configuración flexible**: Horarios, intervalos y recursos personalizables
- **Sistema de colores**: Mapeo de colores por tipo de evento
- **Responsive**: Adaptable a diferentes tamaños de pantalla
- **JavaScript integration**: Eventos interactivos y actualizaciones dinámicas
...
$eventos = [
    [
        'id' => 1,
        'titulo' => 'Corte de cabello - Juan Pérez',
    ...
    [
        'id' => 2,
        'titulo' => 'Tinte completo - María García',
...
$config = [
    'recursos' => [
        'barbero-1' => 'Carlos Martínez',
        'barbero-2' => 'Ana López',
        'barbero-3' => 'Roberto Sánchez'
    ],
    'horaInicio' => '09:00',
    'horaFin' => '19:00',
...
// Horario médico (8 AM - 5 PM)
$configMedico = [
    'horaInicio' => '08:00',
    'horaFin' => '17:00',
    'intervalo' => 15, // 15 minutos por consulta
    'recursos' => [
        'dr-perez' => 'Dr. Pérez - Médico General',
        'dr-garcia' => 'Dra. García - Pediatra',
        'dr-lopez' => 'Dr. López - Cardiología'
    ]
];
...
// Los colores se aplican automáticamente según el campo 'tipo' del evento
$evento = [
...
function calcularPosicionEvento(evento, config) {
    // Lógica para calcular posición absoluta basada en tiempo y recurso
    ...
    // Calcular posición horizontal (recurso)
...
function manejarClickCelda(celda, config) {
    // Obtener información de la celda clickeada
    ...
    const modalHtml = `
        <div class="modal">
            ...
            <form id="form-nueva-cita">
                ...
                <input type="number" name="duracion" placeholder="Duración (minutos)" value="60" required>
...
class SistemaBarberia
{
    public static function mostrarCalendarioDiario($fecha = null)
    {
        ...
        // Renderizar
        echo '<h2>Citas del día: ' . date('d/m/Y', strtotime($fecha)) . '</h2>';
        SchedulerRenderer::render($eventos, $config);
    }
...
class AgendaMedica
{
    ...
    {
        // Configuración para consultorio médico
        $config = [
            'recursos' => [
...
{
    private $eventos;
    private $alturaViewport = 600; // píxeles visibles
...
class CacheScheduler
{
    ...
    {
        // Lógica específica para cargar eventos según tipo
        switch ($tipo) {
            case 'barberia':
...
- `recursoId` no coincide con configuración
...
function validarEventos($eventos, $config)
{
    foreach ($eventos as $evento) {
        ...
        if (!$inicio) {
            error_log("Formato de fecha inválido: " . $evento['horaInicio']);
            continue;
        }
    }
}
...
$eventoEstructurado = [
    'id' => uniqid('evento_'),           // ID único
    'titulo' => 'Título descriptivo',     // Máximo 50 caracteres
    ...
    'descripcion' => 'Texto adicional',   // Máximo 200 caracteres
...
class ValidadorScheduler
{
    public static function validarEvento($evento, $config)
    {
        ...
        if (!self::esFechaValida($evento['horaInicio'])) {
            $errores[] = "Formato de horaInicio inválido";
        }

        if (!self::esFechaValida($evento['horaFin'])) {
            $errores[] = "Formato de horaFin inválido";
        }
        ...
        // Validar recurso
        if (!isset($config['recursos'][$evento['recursoId']])) {
            $errores[] = "Recurso no válido: {$evento['recursoId']}";
        }
...
<Aside type="caution">
⚠️ **Importante**: El SchedulerRenderer requiere CSS personalizado para funcionar correctamente. Asegúrate de incluir los estilos base en tu theme.
</Aside>

<Aside type="tip">
💡 **Tip**: Para aplicaciones complejas, considera implementar lazy loading de eventos y paginación para mejorar el rendimiento con grandes cantidades de datos.
</Aside>
