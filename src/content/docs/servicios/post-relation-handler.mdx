...
description: Manejador especializado para gestionar relaciones de posts con imágenes, galerías y taxonomías
---
...
El `PostRelationHandler` es un servicio especializado que gestiona las relaciones entre posts y otros elementos de WordPress como imágenes destacadas, galerías de medios y términos de taxonomía. Proporciona una interfaz unificada para establecer y mantener estas conexiones de forma automática.

## Funcionalidades Principales

### 🖼️ Gestión de Imágenes Destacadas
- **Asignación automática**: Establece thumbnails desde assets del tema
- **Validación de existencia**: Verifica que los assets existan antes de asignar
- **Soporte para aleatorios**: Imágenes destacadas aleatorias por alias

### 🎞️ Manejo de Galerías
- **Construcción de colecciones**: Crea galerías a partir de arrays de assets
- **Gestión de adjuntos**: Vincula imágenes como hijos del post
- **Galerías automáticas**: Genera galerías por defecto cuando no están definidas

### 🏷️ Control de Taxonomías
- **Asignación de términos**: Vincula posts con categorías y etiquetas
- **Creación automática**: Genera términos inexistentes según definición
- **Manejo de jerarquías**: Soporta taxonomías anidadas
...
$definition = [
    ...
    'metaEntrada' => [
        'category' => 'Noticias, Anuncios',  // Taxonomías
...
```php
'imagenDestacadaAsset' => 'glory::hero-image.jpg'
// Asigna imagen específica del alias 'glory'
```
...
```php
// Vincular imágenes como hijos del post
...
## Integración con PostSyncHandler
...
Configura la galería de medios del post.

### `setTerms(array $definicion): void`
Asigna términos de taxonomía al post.
...
$aliasesDisponibles = [
    'glory' => '/assets/images/',      // Imágenes principales
...
### Depuración de Taxonomías

```php
// Verificar asignaciones
$assignedTerms = wp_get_object_terms($postId, $taxonomy);
GloryLogger::info("Términos asignados", [
    'taxonomy' => $taxonomy,
...
### Configuración de Página de Producto
...
$productoDefinition = [
    ...
    'metaEntrada' => [
        'product_cat' => 'Electrónicos, Premium',
...
$blogDefinition = [
    ...
    'contenido' => 'Análisis de tendencias tecnológicas...',
    'imagenDestacadaAsset' => 'random::colors',  // Imagen aleatoria
    'metaEntrada' => [
        'category' => 'Tecnología, Tendencias',
        'post_tag' => '2024, innovación, futuro'
    ]
];
...
```php
// Verificar taxonomía existe antes de asignar
if (!taxonomy_exists($taxonomy)) {
    GloryLogger::warning("Taxonomía no existe", [
        'taxonomy' => $taxonomy,
        'postId' => $this->postId
    ]);
    return;
...
```php
// Para múltiples posts, procesar en lotes
...
```php
// AssetsUtility mantiene caché interno
...
$formulario
    ->campo('titulo', 'text', 'Título')
    ->campo('imagen_destacada', 'asset_selector', 'Imagen Destacada')
    ->campo('categorias', 'taxonomy_selector', 'Categorías')
    ->procesar();
    ...
    if (!empty($definition['galeriaAssets'])) {
        foreach ($definition['galeriaAssets'] as $asset) {
            if (!AssetsUtility::assetExists($asset)) {
                $errores[] = "Asset de galería no existe: {$asset}";
            }
        }
    }
