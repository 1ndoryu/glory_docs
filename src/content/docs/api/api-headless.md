...
                    "id": 1,
                    "titulo": "Mi Sample",
                    "id_autor": 2, // <-- Clave corregida a snake_case
                    "autor": {
                      "id": 2,
                      "nombre_usuario": "artista_uno",
                      "nombre_mostrado": "Artista Uno",
...
-   **Nota sobre Metadata:** El campo `metadata` contendrá un objeto JSON complejo con datos extraídos por una IA (ej: en `metadata.ai_analysis`). La estructura de este objeto puede variar y los clientes deben estar preparados para manejar claves y valores diversos.
...
*Para los demás endpoints (`GET /users`, `POST /users`, `DELETE /users/{id}`), consulta la matriz de permisos. Requieren rol de `admin` o ser el propietario del perfil.*
