...
- En **producción** el override actúa como **valor por defecto**: si el cliente final / administrador del sitio cambió la opción desde el panel, ese valor guardado en la BD prevalece.
...
- En producción, si el cliente final configuró la feature como `false` en el panel, `isActive` devolverá `false` aunque el código llame a `enable()` (el cliente final tiene la última palabra en producción).
