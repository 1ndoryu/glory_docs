...
description: Gestor de acciones interactivas sobre elementos de contenido con eliminación, submenús y eventos contextuales
---
...
El sistema de acciones de contenido proporciona una interfaz interactiva para gestionar elementos de contenido (posts, custom post types) mediante diferentes tipos de eventos como doble clic, clic derecho o pulsación larga. Permite eliminación, edición y otras acciones contextuales de forma agnóstica.
...
<div class="content-container" data-accion-evento="dblclick">
    <!-- Doble clic en cualquier elemento hijo ejecutará la acción -->
</div>
```
...
<div class="content-container"
    ...
    <!-- Clic derecho muestra submenú -->
</div>
```

#### Pulsación Larga (`longpress`)
Para dispositivos táctiles - mantiene presionado 500ms.

```html
<div class="content-container" data-accion-evento="longpress">
    <!-- Mantener presionado 500ms activa la acción -->
</div>
```
...
Elimina el elemento vía AJAX y actualiza la interfaz.

```javascript
// Acción automática al activar evento
// Envía: { accion: 'eliminar', postId, postType, modo: 'trash' }
```

#### Submenús Personalizables
Menús contextuales que aparecen en la posición del cursor.

```html
<!-- Submenú generado automáticamente -->
...
### Gestión Jerárquica
Soporte para elementos padre/hijo con limpieza automática.
...
- **Active**: Feedback durante la ejecución de acciones
- **Loading**: Estados de carga durante operaciones AJAX

### Submenús
- **Posicionamiento**: Aparecen en la posición del cursor
- **Overlay**: Usan el sistema de fondos para bloqueo
- **Animaciones**: Transiciones suaves de entrada/salida

### Feedback Visual
- **Confirmación**: Mensajes de éxito/error tras acciones
- **Transiciones**: Elementos se eliminan con animación
- **Estados**: Loading states durante operaciones asíncronas
...
<!-- Submenú generado dinámicamente -->
...
<div data-post-type="producto">
    <!-- Operaciones se ejecutarán sobre tipo 'producto' -->
</div>
```
...
Habilita/deshabilita submenús contextuales.
...
Número de elementos por página para refresco.
...
Función de callback para plantillas personalizadas.
...
#### Por ID
```html
<div id="post-123"> <!-- Detectado automáticamente -->
```

#### Por Clase
```html
<div class="post-id-456"> <!-- ID extraído de clase -->
```
...
Encuentra el elemento de contenido más cercano.
...
Muestra el submenú contextual para un elemento.
...
#### Sistema de Fondos
```javascript
// Los submenús usan mostrarFondo/ocultarFondo automáticamente
```
...
### Galería de Imágenes con Eliminación
```html
    ...
    <div class="admin-item" id="post-123">
        <div class="item-header">
            ...
            <div class="item-actions">
                <button class="context-menu-trigger">⋯</button>
            </div>
        </div>
...
- ✅ **Agnóstico**: Funciona con cualquier tipo de contenido
- ✅ **Responsive**: Compatible con dispositivos táctiles
- ✅ **Accesibilidad**: Soporte para navegación por teclado
- ✅ **Event-driven**: Sistema de eventos desacoplado
- ✅ **AJAX**: Integración completa con gloryAjax
- ✅ **Jerárquico**: Soporte para estructuras padre/hijo
- ✅ **Extensible**: Fácil añadir nuevas acciones
- ✅ **Fallback**: Funciona sin dependencias externas
...
3. **Submenús no aparecen**: Confirmar `data-submenu-enabled="1"`
4. **Eventos no responden**: Verificar `data-accion-evento`
5. **AJAX falla**: Comprobar configuración de `gloryAjax`
...
// Verificar integración con AJAX
...
### Troubleshooting de Submenús
```javascript
// Verificar creación de submenús
function verificarSubmenus() {
    const items = document.querySelectorAll('[data-submenu]');
    console.log('Items con submenú:', items.length);

    items.forEach(item => {
        const menuId = item.dataset.submenu;
        const menu = document.getElementById(menuId);
        console.log('Menú', menuId, 'existe:', !!menu);
        if (menu) {
            console.log('Menú visible:', menu.style.display !== 'none');
        }
    });
}
