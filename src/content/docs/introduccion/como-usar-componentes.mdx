---
title: Cómo Usar Componentes
description: Guía práctica para utilizar los componentes reutilizables de Glory Framework
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

# 🧱 Cómo Usar Componentes de Glory

Glory incluye una colección de componentes reutilizables que simplifican tareas comunes de desarrollo web. Estos componentes están diseñados para ser agnósticos y se pueden usar tanto en temas como en plugins.

## 📋 Tabla de Contenidos

- [🎨 Componentes de UI (Interfaz de Usuario)](#componentes-de-ui-interfaz-de-usuario)
  - [Sistema de Modales](#sistema-de-modales)
  - [Sistema de Alertas](#sistema-de-alertas)
  - [Previsualización de Archivos](#previsualizacion-de-archivos)
  - [Pestañas y Submenús](#pestanas-y-submenus)

- [📋 Componentes de Renderizado](#componentes-de-renderizado)
  - [ContentRender - Mostrar Listas de Contenido](#contentrender---mostrar-listas-de-contenido)
  - [TermRender - Mostrar Términos de Taxonomía](#termrender---mostrar-terminos-de-taxonomia)
  - [BusquedaRenderer - Búsqueda Predictiva](#busquedarenderer---busqueda-predictiva)
  - [LogoRenderer - Mostrar Logo del Sitio](#logorenderer---mostrar-logo-del-sitio)

- [📝 Componentes de Formularios](#componentes-de-formularios)
  - [FormBuilder - Constructor de Formularios](#formbuilder---constructor-de-formularios)
  - [Formulario de Registro de Usuario](#formulario-de-registro-de-usuario)

- [🎨 Componentes Visuales](#componentes-visuales)
  - [BadgeList - Lista de Badges](#badgelist---lista-de-badges)
  - [DataGridRenderer - Tabla de Datos Interactiva](#datagridrenderer---tabla-de-datos-interactiva)
  - [PaginationRenderer - Paginación Personalizada](#paginationrenderer---paginacion-personalizada)
  - [ThemeToggle - Interruptor de Tema Claro/Oscuro](#themetoggle---interruptor-de-tema-clarooscuro)

- [🚀 Navegación AJAX](#navegacion-ajax)
  - [Configuración Básica](#configuracion-basica)
  - [Exclusiones de AJAX](#exclusiones-de-ajax)
  - [Eventos Personalizados](#eventos-personalizados)

- [🎯 Mejores Prácticas](#mejores-practicas)
  - [Organización del Código](#organizacion-del-codigo)
  - [Optimización de Rendimiento](#optimizacion-de-rendimiento)
  - [Accesibilidad](#accesibilidad)

## 🎨 Componentes de UI (Interfaz de Usuario)

### Sistema de Modales

Los modales permiten mostrar contenido en ventanas emergentes sin recargar la página.

#### Uso Básico

```html
<!-- Botón que abre el modal -->
<button class="glory-modal-trigger" data-modal="mi-modal">
    Abrir Modal
</button>

<!-- Contenido del modal -->
<div id="mi-modal" class="glory-modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Título del Modal</h3>
            <button class="modal-close">&times;</button>
        </div>
        <div class="modal-body">
            <p>Contenido del modal aquí...</p>
        </div>
    </div>
</div>
```

#### Modal con Formulario

```php
<?php
// En tu template PHP
?>
<div id="modal-contacto" class="glory-modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Contactar Vendedor</h3>
            <button class="modal-close">&times;</button>
        </div>
        <div class="modal-body">
            <?php
            use Glory\Components\FormBuilder;

            echo FormBuilder::inicio([
                'atributos' => ['id' => 'form-contacto-modal']
            ]);

            echo FormBuilder::campoTexto([
                'nombre' => 'nombre',
                'label' => 'Tu Nombre',
                'requerido' => true
            ]);

            echo FormBuilder::campoEmail([
                'nombre' => 'email',
                'label' => 'Email',
                'requerido' => true
            ]);

            echo FormBuilder::botonEnviar([
                'accion' => 'contactarVendedor',
                'texto' => 'Enviar Mensaje'
            ]);

            echo FormBuilder::fin();
            ?>
        </div>
    </div>
</div>
```

#### Control desde JavaScript

```javascript
// Abrir modal programáticamente
document.querySelector('.modal-trigger').addEventListener('click', function() {
    window.gloryModal.open('mi-modal');
});

// Cerrar modal
window.gloryModal.close('mi-modal');

// Con callbacks
window.gloryModal.open('mi-modal', {
    onOpen: function() {
        console.log('Modal abierto');
    },
    onClose: function() {
        console.log('Modal cerrado');
    }
});
```

### Sistema de Alertas

Reemplaza las alertas nativas del navegador por notificaciones elegantes.

```javascript
// Alerta básica
window.gloryAlert.show('Operación completada correctamente', 'success');

// Tipos disponibles: 'success', 'error', 'warning', 'info'

// Alerta con opciones
window.gloryAlert.show('¿Estás seguro?', 'warning', {
    confirmText: 'Sí, continuar',
    cancelText: 'Cancelar',
    onConfirm: function() {
        // Acción al confirmar
        realizarAccion();
    },
    onCancel: function() {
        // Acción al cancelar
        console.log('Cancelado');
    }
});
```

### Previsualización de Archivos

Gestiona la subida y previsualización de archivos con drag & drop.

```html
<div class="glory-file-preview" data-max-files="5" data-allowed-types="jpg,png,pdf">
    <div class="preview-zone">
        <p>Arrastra archivos aquí o <span class="file-link">haz clic para seleccionar</span></p>
        <input type="file" multiple accept=".jpg,.png,.pdf" style="display: none;">
    </div>
    <div class="preview-list"></div>
</div>
```

### Pestañas y Submenús

```html
<!-- Pestañas -->
<div class="glory-tabs">
    <div class="tabs-header">
        <button class="tab-button active" data-tab="tab1">Información</button>
        <button class="tab-button" data-tab="tab2">Configuración</button>
        <button class="tab-button" data-tab="tab3">Avanzado</button>
    </div>
    <div class="tabs-content">
        <div class="tab-pane active" id="tab1">
            <p>Contenido de la primera pestaña</p>
        </div>
        <div class="tab-pane" id="tab2">
            <p>Contenido de la segunda pestaña</p>
        </div>
        <div class="tab-pane" id="tab3">
            <p>Contenido de la tercera pestaña</p>
        </div>
    </div>
</div>
```

## 📋 Componentes de Renderizado

### ContentRender - Mostrar Listas de Contenido

Uno de los componentes más poderosos para mostrar posts, páginas o tipos de contenido personalizados.

#### Lista Básica de Posts

```php
<?php
use Glory\Components\ContentRender;

// Mostrar los 5 posts más recientes
ContentRender::print('post', [
    'publicacionesPorPagina' => 5,
    'paginacion' => false
]);
?>
```

#### Blog con Paginación AJAX

```php
<?php
ContentRender::print('post', [
    'publicacionesPorPagina' => 10,
    'paginacion' => true,
    'paginacionAjax' => true,
    'plantillaCallback' => 'plantillaPostPersonalizada'
]);
?>

<script>
// Función de callback para la paginación AJAX
function plantillaPostPersonalizada(post) {
    return `
        <article class="post-card">
            <h2>${post.post_title}</h2>
            <div class="post-meta">
                <span>Por ${post.author_name}</span>
                <span>${post.post_date}</span>
            </div>
            <div class="post-excerpt">
                ${post.post_excerpt}
            </div>
            <a href="${post.permalink}">Leer más</a>
        </article>
    `;
}
</script>
```

#### Catálogo de Productos

```php
<?php
ContentRender::print('producto', [
    'queryArgs' => [
        'meta_query' => [
            [
                'key' => 'en_stock',
                'value' => '1',
                'compare' => '='
            ]
        ],
        'orderby' => 'meta_value_num',
        'meta_key' => 'precio',
        'order' => 'ASC'
    ],
    'publicacionesPorPagina' => 12,
    'paginacion' => true,
    'plantillaCallback' => 'plantillaProducto'
]);
?>

<script>
function plantillaProducto(producto) {
    return `
        <div class="producto-card">
            <img src="${producto.thumbnail}" alt="${producto.post_title}">
            <h3>${producto.post_title}</h3>
            <p class="precio">${producto.meta.precio}€</p>
            <button onclick="agregarAlCarrito(${producto.ID})">
                Añadir al carrito
            </button>
        </div>
    `;
}
</script>
```

### TermRender - Mostrar Términos de Taxonomía

```php
<?php
use Glory\Components\TermRender;

// Mostrar todas las categorías
TermRender::print('category', [
    'hide_empty' => false,
    'orderby' => 'count',
    'order' => 'DESC'
]);
?>
```

#### Nube de Etiquetas Personalizada

```php
<?php
TermRender::print('post_tag', [
    'number' => 20,
    'orderby' => 'count',
    'order' => 'DESC',
    'plantillaCallback' => 'plantillaEtiqueta'
]);
?>

<script>
function plantillaEtiqueta(tag) {
    const fontSize = Math.min(8 + tag.count * 2, 24);
    return `<a href="${tag.link}" style="font-size: ${fontSize}px;">${tag.name}</a>`;
}
</script>
```

### BusquedaRenderer - Búsqueda Predictiva

Implementa búsqueda en tiempo real sin necesidad de configuración compleja.

```html
<!-- Campo de búsqueda básico -->
<input type="text"
       class="glory-search"
       data-tipos="post,page,producto"
       data-cantidad="5"
       data-target="#resultados-busqueda"
       placeholder="Buscar contenido...">

<div id="resultados-busqueda"></div>
```

#### Búsqueda Avanzada con Filtros

```html
<div class="busqueda-avanzada">
    <input type="text"
           class="glory-search"
           data-tipos="producto"
           data-cantidad="8"
           data-target="#resultados-productos"
           data-filtros='{"categoria": "electronica", "precio_max": 1000}'
           placeholder="Buscar productos...">

    <div class="filtros-activos">
        <span class="filtro" data-filtro="categoria" data-valor="electronica">
            Electrónica ✕
        </span>
        <span class="filtro" data-filtro="precio_max" data-valor="1000">
            Máximo 1000€ ✕
        </span>
    </div>
</div>

<div id="resultados-productos" class="resultados-grid"></div>
```

### LogoRenderer - Mostrar Logo del Sitio

```php
<?php
use Glory\Components\LogoRenderer;

// Logo básico
echo LogoRenderer::render();

// Logo con opciones
echo LogoRenderer::render([
    'width' => '200px',
    'filter' => 'white',
    'link' => true
]);
?>
```

#### Como shortcode en el editor

```
[theme_logo width="150px" filter="black"]
```

## 📝 Componentes de Formularios

### FormBuilder - Constructor de Formularios

```php
<?php
use Glory\Components\FormBuilder;

// Inicio del formulario
echo FormBuilder::inicio([
    'atributos' => [
        'id' => 'form-contacto',
        'class' => 'formulario-contacto'
    ]
]);

// Campo de texto
echo FormBuilder::campoTexto([
    'nombre' => 'nombre_completo',
    'label' => 'Nombre Completo',
    'requerido' => true,
    'placeholder' => 'Ingresa tu nombre'
]);

// Campo de email
echo FormBuilder::campoEmail([
    'nombre' => 'email',
    'label' => 'Correo Electrónico',
    'requerido' => true
]);

// Campo de selección
echo FormBuilder::campoSelect([
    'nombre' => 'asunto',
    'label' => 'Asunto',
    'opciones' => [
        '' => 'Selecciona un asunto',
        'consulta' => 'Consulta General',
        'soporte' => 'Soporte Técnico',
        'ventas' => 'Información de Ventas'
    ],
    'requerido' => true
]);

// Campo de textarea
echo FormBuilder::campoTextarea([
    'nombre' => 'mensaje',
    'label' => 'Mensaje',
    'requerido' => true,
    'filas' => 5
]);

// Campo de checkbox
echo FormBuilder::campoCheckbox([
    'nombre' => 'acepto_terminos',
    'label' => 'Acepto los términos y condiciones',
    'requerido' => true
]);

// Botón de envío
echo FormBuilder::botonEnviar([
    'accion' => 'procesarFormularioContacto',
    'texto' => 'Enviar Mensaje',
    'clases' => 'btn btn-primary btn-large'
]);

// Fin del formulario
echo FormBuilder::fin();
?>
```

### Formulario de Registro de Usuario

```php
<?php
echo FormBuilder::inicio([
    'atributos' => ['id' => 'form-registro']
]);

echo FormBuilder::campoTexto([
    'nombre' => 'user_login',
    'label' => 'Nombre de usuario',
    'requerido' => true
]);

echo FormBuilder::campoEmail([
    'nombre' => 'user_email',
    'label' => 'Correo electrónico',
    'requerido' => true
]);

echo FormBuilder::campoPassword([
    'nombre' => 'user_pass',
    'label' => 'Contraseña',
    'requerido' => true,
    'minlength' => 8
]);

echo FormBuilder::campoPassword([
    'nombre' => 'user_pass_confirm',
    'label' => 'Confirmar contraseña',
    'requerido' => true
]);

echo FormBuilder::botonEnviar([
    'accion' => 'registrarUsuario',
    'texto' => 'Crear cuenta'
]);

echo FormBuilder::fin();
?>
```

## 🎨 Componentes Visuales

### BadgeList - Lista de Badges

```php
<?php
use Glory\Components\BadgeList;

// Badges básicos
BadgeList::render([
    'etiquetas' => ['Nuevo', 'Destacado', 'Oferta'],
    'tipo' => 'success'
]);

// Badges con enlaces
BadgeList::render([
    'etiquetas' => [
        ['texto' => 'Tecnología', 'url' => '/categoria/tecnologia'],
        ['texto' => 'Innovación', 'url' => '/categoria/innovacion']
    ],
    'tipo' => 'info'
]);
?>
```

### DataGridRenderer - Tabla de Datos Interactiva

```php
<?php
use Glory\Components\DataGridRenderer;

// Datos de ejemplo
$productos = [
    ['id' => 1, 'nombre' => 'Producto A', 'precio' => 29.99, 'stock' => 150],
    ['id' => 2, 'nombre' => 'Producto B', 'precio' => 49.99, 'stock' => 75],
    // ... más datos
];

DataGridRenderer::render([
    'datos' => $productos,
    'columnas' => [
        'id' => ['titulo' => 'ID', 'ancho' => '80px'],
        'nombre' => ['titulo' => 'Producto', 'ordenable' => true],
        'precio' => ['titulo' => 'Precio', 'tipo' => 'moneda', 'ordenable' => true],
        'stock' => ['titulo' => 'Stock', 'tipo' => 'numero', 'ordenable' => true]
    ],
    'paginacion' => true,
    'busqueda' => true,
    'acciones' => [
        'editar' => ['url' => '/admin/editar-producto/{id}', 'texto' => 'Editar'],
        'eliminar' => ['accion' => 'eliminarProducto', 'texto' => 'Eliminar', 'confirmar' => true]
    ]
]);
?>
```

### PaginationRenderer - Paginación Personalizada

```php
<?php
use Glory\Components\PaginationRenderer;

// Para WordPress Query
global $wp_query;
PaginationRenderer::render([
    'total' => $wp_query->found_posts,
    'por_pagina' => $wp_query->query_vars['posts_per_page'],
    'actual' => get_query_var('paged') ?: 1,
    'base_url' => get_pagenum_link(1),
    'estilo' => 'modern', // 'classic', 'modern', 'minimal'
    'mostrar_info' => true
]);
?>
```

### ThemeToggle - Interruptor de Tema Claro/Oscuro

```html
<div class="theme-toggle-container">
    <button id="theme-toggle" aria-label="Cambiar tema">
        <span class="toggle-icon">🌙</span>
        <span class="toggle-text">Tema Oscuro</span>
    </button>
</div>
```

```javascript
// Inicializar el toggle
document.addEventListener('DOMContentLoaded', function() {
    const toggle = document.getElementById('theme-toggle');

    // Función para cambiar tema
    function toggleTheme() {
        const currentTheme = document.documentElement.getAttribute('data-theme');
        const newTheme = currentTheme === 'dark' ? 'light' : 'dark';

        document.documentElement.setAttribute('data-theme', newTheme);
        localStorage.setItem('theme', newTheme);

        // Actualizar icono y texto
        updateToggleButton(newTheme);
    }

    function updateToggleButton(theme) {
        const icon = toggle.querySelector('.toggle-icon');
        const text = toggle.querySelector('.toggle-text');

        if (theme === 'dark') {
            icon.textContent = '☀️';
            text.textContent = 'Tema Claro';
        } else {
            icon.textContent = '🌙';
            text.textContent = 'Tema Oscuro';
        }
    }

    // Cargar tema guardado
    const savedTheme = localStorage.getItem('theme') || 'light';
    document.documentElement.setAttribute('data-theme', savedTheme);
    updateToggleButton(savedTheme);

    // Event listener
    toggle.addEventListener('click', toggleTheme);
});
```

## 🚀 Navegación AJAX

Glory incluye navegación AJAX por defecto, convirtiendo tu sitio en una SPA (Single Page Application).

### Configuración Básica

La navegación AJAX se activa automáticamente. Solo necesitas:

```html
<!-- Tu navegación normal -->
<nav>
    <a href="/inicio">Inicio</a>
    <a href="/productos">Productos</a>
    <a href="/contacto">Contacto</a>
</nav>

<!-- Contenedor donde se carga el contenido -->
<main id="main-content">
    <!-- El contenido se carga aquí vía AJAX -->
</main>
```

### Exclusiones de AJAX

Si algunas páginas no deben usar AJAX:

```javascript
// En tu JavaScript personalizado
window.ajaxNavigation = {
    excludePatterns: [
        '/admin',
        '/wp-login',
        '/carrito',
        '/checkout'
    ]
};
```

### Eventos Personalizados

```javascript
document.addEventListener('gloryAjaxNav:loading', function() {
    // Mostrar spinner de carga
    showLoadingSpinner();
});

document.addEventListener('gloryAjaxNav:loaded', function(e) {
    // Ocultar spinner
    hideLoadingSpinner();

    // Actualizar elementos específicos
    updateSidebar();
    updateBreadcrumbs();
});

document.addEventListener('gloryAjaxNav:error', function(e) {
    console.error('Error de navegación:', e.detail.error);
});
```

## 🎯 Mejores Prácticas

### Organización del Código

1. **Agrupa componentes relacionados** en la misma sección
2. **Usa IDs únicos** para elementos interactivos
3. **Configura callbacks** para personalizar comportamiento
4. **Valida datos** antes de enviar formularios

### Optimización de Rendimiento

```javascript
// Lazy loading para componentes pesados
document.addEventListener('DOMContentLoaded', function() {
    // Inicializar solo cuando sea necesario
    const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                initializeHeavyComponent(entry.target);
                observer.unobserve(entry.target);
            }
        });
    });

    document.querySelectorAll('.componente-pesado').forEach(el => {
        observer.observe(el);
    });
});
```

### Accesibilidad

```php
// Asegurar que los componentes sean accesibles
echo FormBuilder::campoTexto([
    'nombre' => 'email',
    'label' => 'Correo electrónico',
    'requerido' => true,
    'atributos' => [
        'aria-describedby' => 'email-help',
        'autocomplete' => 'email'
    ]
]);
echo '<span id="email-help" class="form-help">Usaremos este email solo para contactarte</span>';
```

¡Estos componentes te permiten construir interfaces complejas de manera rápida y consistente! Cada componente está diseñado para ser flexible y extensible, adaptándose a tus necesidades específicas.
