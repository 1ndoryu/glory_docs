...
description: Sistema de organización automática de elementos en columnas estilo muro de ladrillos con posicionamiento absoluto
---
...
El sistema Masonry Row Major proporciona un layout dinámico que organiza automáticamente los elementos en columnas estilo muro de ladrillos, calculando posiciones absolutas para crear un efecto visual de altura variable. Es completamente agnóstico y se adapta automáticamente a cambios de contenido y tamaño de ventana.
...
<div id="mi-masonry-container" style="position: relative;">
    ...
    <div class="seccionComponente">Elemento 3 - más texto aquí</div>
...
/* Clase aplicada automáticamente */
.masonry-js .seccionComponente {
    position: absolute;
    /* width calculado dinámicamente */
    /* transform calculado dinámicamente */
}
```

## 📋 Funcionalidades

### Layout Automático por Filas
Organiza elementos en orden de fila mayoritaria, distribuyéndolos en columnas para minimizar la altura total.
...
Analiza el CSS `grid-template-columns` para determinar el número de columnas automáticamente.
...
/* JavaScript detecta automáticamente 4 columnas */
```

### Recálculo Dinámico
Se adapta automáticamente a cambios en el DOM, redimensionamiento de ventana y carga de imágenes.

```javascript
// Eventos que activan recálculo automático:
...
<div class="seccionComponente" style="display: none;">Este también</div>
<div class="seccionComponente">Este sí se incluye</div>
```
...
- **Ancho de columnas**: `(contenedor - gaps) / número_columnas`
- **Posición X**: `columna_index * (ancho_columna + gap)`
- **Posición Y**: `altura_acumulada_columna`
- **Altura contenedor**: `máxima_altura_columnas`
...
/* Evitar durante el cálculo inicial */
...
<div id="masonry-container" class="masonry-js" style="position: relative; height: 450px;">
    ...
    <!-- Más elementos posicionados automáticamente -->
</div>
```
...
#### Parámetros de `inicializarMasonryRowMajor`
```javascript
const masonry = Glory.inicializarMasonryRowMajor(selector, columnas);

// selector: string - Selector CSS del contenedor
// columnas: number - Número de columnas (opcional, se detecta automáticamente)
```

#### Ejemplos de Inicialización
```javascript
// Con detección automática de columnas
...
#### `Glory.recalcularMasonryRowMajor()`
Fuerza un recálculo inmediato del layout.

```javascript
// Después de añadir elementos dinámicamente
...
- **Contenedor**: Cualquier selector válido (`#id`, `.clase`, `[atributo]`)
- **Elementos**: `.seccionComponente` (configurable internamente)

#### Clases CSS Aplicadas
- **`.masonry-js`**: Aplicada al contenedor durante inicialización
- **Posicionamiento**: `position: absolute` en cada elemento
- **Dimensiones**: `width` calculada dinámicamente
...
.masonry-container .seccionComponente {
    ...
    margin-bottom: 0; /* Evitar márgenes que interfieran */
...
Analiza `grid-template-columns` para determinar número de columnas.
...
#### `layout()`
Función principal que calcula y aplica posiciones.

```javascript
function layout() {
    // 1. Obtener elementos visibles
    // 2. Calcular dimensiones de columnas
    // 3. Distribuir elementos por altura mínima
...
#### MutationObserver
Monitorea cambios en el DOM para recálculos automáticos.
...
#### Event Listeners
- **`load`**: Layout inicial cuando la página carga
- **`resize`**: Recálculo al cambiar tamaño de ventana
...
- **Imágenes**: Layout inmediato tras carga
...
<div id="galeria-masonry" class="galeria-container">
    <div class="seccionComponente imagen-item">
        ...
        <div class="caption">Descripción de la foto 1</div>
    </div>

    <div class="seccionComponente imagen-item">
        <img src="foto2.jpg" alt="Foto 2">
        <div class="caption">Foto más alta que las demás</div>
    </div>

    <!-- Más imágenes con alturas variables -->
</div>
```
...
<section id="productos-masonry" class="productos-grid">
    ...
    <article class="seccionComponente producto-card">
        ...
        <p>Descripción más larga con más detalles sobre las características del producto</p>
...
<div id="blog-masonry" class="blog-posts">
    ...
    <div class="seccionComponente post-card featured">
        <h2>Título Más Largo Que Requiere Más Espacio</h2>
        <p>Extracto mucho más largo que incluye más contenido y detalles sobre el tema tratado en el artículo...</p>
...
```javascript
// Inicialización con columnas responsive
...
- ✅ **Agnóstico**: No depende de frameworks específicos
- ✅ **Responsive**: Se adapta automáticamente al tamaño de ventana
- ✅ **Performance**: Cálculos optimizados y debounced
- ✅ **CSS Grid**: Detecta automáticamente configuración de columnas
- ✅ **Imágenes**: Espera carga completa antes de calcular
- ✅ **Dinámico**: Maneja adición/eliminación de elementos
- ✅ **Event-driven**: Integración con eventos personalizados
- ✅ **Limpieza**: Método desconectar para liberar recursos
...
2. **Alturas incorrectas**: Asegurar que imágenes tengan `display: block` o estén cargadas
3. **No responsive**: Verificar que el CSS permita recálculos en resize
4. **Performance issues**: Reducir número de columnas o aumentar debouncing
...
function reinicializarMasonry() {
    resetMasonry();
    // Re-inicializar según sea necesario
