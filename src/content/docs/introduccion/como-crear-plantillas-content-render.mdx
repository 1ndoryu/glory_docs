...
ContentRender::print('post', [
  ...
  'plantillaCallback' => function(\WP_Post $post, string $itemClass){
    // O usa TemplateRegistry::get('mi_plantilla')
    mi_plantilla($post, $itemClass);
  },
  'argumentosConsulta' => [ 'orderby' => 'date', 'order' => 'DESC' ],
]);
```
